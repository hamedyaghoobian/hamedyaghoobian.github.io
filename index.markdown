...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // sunrise/moonrise plays entirely off-screen. Below the layout's own
    // mobile breakpoint, body.mashhad-active instead plays the
    // mashhad-local-rise/-horizon keyframes (main.scss) right next to the
    // word itself — same event, no scroll. Timing lives entirely in those
    // keyframes, not here: this just starts the class and clears it again
    // on animationend, so the two can't drift out of sync the way a
    // hand-tuned setTimeout duration could.
    ...
    if (mashhad) {
      mashhad.addEventListener('click', () => {
        ...
        // Force a reflow between removing and re-adding the class so a
        // repeat tap restarts the animation — re-adding a class that's
        // already present doesn't retrigger it on its own.
      ...
      document.body.addEventListener('animationend', e => {
        if (e.animationName === 'mashhad-local-rise') {
          document.body.classList.remove('mashhad-active');
        }
      });
    }
